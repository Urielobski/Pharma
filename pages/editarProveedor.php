...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["name"]) && isset($_POST["direccion"]) && isset($_POST["tel"])) {
    ...
    $stmt->bindParam(':direccion', $cantidad);
    $stmt->bindParam(':tel', $lote);
    $stmt->bindParam(':id', $idToUpdate);

    if ($stmt->execute()) {
        $messageError = 'Successfully updated client';
        header("Location: ./proveedores.php");
        exit();
    } else {
        $messageError = 'Sorry there must have been an issue updating the client';
    }
}
?>
<!DOCTYPE html>
...
<body class="flex">
    <nav id="contMenu" class="w-full h-24 min-w-96 bg-slate-900 flex justify-around items-center px-10 shadow-2xl fixed">
        ...
        <div class="w-1/3 flex justify-start items-center ">
            <h1 class="w-full text-center text-3xl text-white"> Proveedor</h1>
        </div>
    ...
    <nav class="w-1/6 h-screen bg-slate-300 shadow-2xl flex flex-col justify-center items-center">
        <ul class="w-full">
            ...
            <li class="w-full flex">
                <a class="w-full p-5 bg-slate-400 hover:bg-slate-400 flex items-center gap-4" href="#">
                    <svg class="w-10" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
    ...
    <main class="w-5/6 h-screen  bg-slate-700 flex flex-col justify-center items-center">
        ...
        $consulta = 'SELECT * FROM proveedores WHERE id =' . $idToUpdate;
        ...
        ?>
            <form method="POST" action="" class="w-5/6 mx-auto bg-slate-900 p-10 mt-20 rounded-3xl">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input value="<?php echo $row['nombre'] ?>" name="name" type="text" name="floating_email" id="floating_email" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                    <label for="floating_email" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        Nombre
                    </label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input value="<?php echo $row['direccion'] ?>" name="direccion" type="text" name="floating_password" id="floating_password" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                    <label for="floating_password" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        Direccion
                    </label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input value="<?php echo $row['tel'] ?>" name="tel" type="text" name="repeat_password" id="floating_repeat_password" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                    <label for="floating_repeat_password" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        Tel
                    </label>
                </div>                
                <button type="Actualizar" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Actualizar</button>
            </form>
        <?php
        }
        ?>
